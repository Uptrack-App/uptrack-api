...
- [x] Auth key generated (REMOVED_TAILSCALE_AUTH_KEY, expires Jan 28, 2026)
...
- [ ] Test SSH to india-w: `ssh root@REMOVED_IP`
- [ ] Test SSH to eu-a: `ssh root@REMOVED_IP`
- [ ] Test SSH to eu-b: `ssh root@REMOVED_IP`
- [ ] Test SSH to eu-c: `ssh root@REMOVED_IP`
...
### Deploy to india-w
- [ ] Run installation script: `cat scripts/install-tailscale-debian.sh | ssh root@REMOVED_IP 'bash -s india-w tskey-auth-...'`
- [ ] Verify node appears in Tailscale admin console
- [ ] Verify Tailscale IP: `ssh root@REMOVED_IP 'sudo tailscale ip -4'`
- [ ] Test ping from india-s to india-w (should be <10ms)

### Deploy to eu-a
- [ ] Run installation script: `cat scripts/install-tailscale-debian.sh | ssh root@REMOVED_IP 'bash -s eu-a tskey-auth-...'`
- [ ] Verify node appears in Tailscale admin console
- [ ] Verify Tailscale IP: `ssh root@REMOVED_IP 'sudo tailscale ip -4'`

### Deploy to eu-b
- [ ] Run installation script: `cat scripts/install-tailscale-debian.sh | ssh root@REMOVED_IP 'bash -s eu-b tskey-auth-...'`
- [ ] Verify node appears in Tailscale admin console
- [ ] Verify Tailscale IP: `ssh root@REMOVED_IP 'sudo tailscale ip -4'`

### Deploy to eu-c
- [ ] Run installation script: `cat scripts/install-tailscale-debian.sh | ssh root@REMOVED_IP 'bash -s eu-c tskey-auth-...'`
- [ ] Verify node appears in Tailscale admin console
- [ ] Verify Tailscale IP: `ssh root@REMOVED_IP 'sudo tailscale ip -4'`
...
- [ ] Verify india-w: `ssh root@REMOVED_IP 'sudo tailscale ip -4'` shows `100.64.1.11`
- [ ] Verify eu-a: `ssh root@REMOVED_IP 'sudo tailscale ip -4'` shows `100.64.1.1`
- [ ] Verify eu-b: `ssh root@REMOVED_IP 'sudo tailscale ip -4'` shows `100.64.1.2`
- [ ] Verify eu-c: `ssh root@REMOVED_IP 'sudo tailscale ip -4'` shows `100.64.1.3`

## Phase 4: Connectivity Verification (20 minutes)

### Ping Tests - EU Internal
- [ ] From eu-a, ping eu-b: `ssh root@REMOVED_IP 'ping -c 3 100.64.1.2'` (expect <20ms)
- [ ] From eu-a, ping eu-c: `ssh root@REMOVED_IP 'ping -c 3 100.64.1.3'` (expect <20ms)
- [ ] From eu-b, ping eu-c: `ssh root@REMOVED_IP 'ping -c 3 100.64.1.3'` (expect <20ms)
...
- [ ] From eu-a, ping india-s: `ssh root@REMOVED_IP 'ping -c 3 100.64.1.10'` (expect ~150ms)
- [ ] From eu-a, ping india-w: `ssh root@REMOVED_IP 'ping -c 3 100.64.1.11'` (expect ~150ms)
