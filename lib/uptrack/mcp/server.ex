defmodule Uptrack.MCP.Server do
  ...
  def handle_message(message_json, org_id) do
    case Jason.decode(message_json) do
      {:ok, message} ->
        response = handle_mcp_message(message, org_id)
  ...
  defp handle_mcp_message(%{"jsonrpc" => "2.0", "method" => "initialize", "id" => id}, _org_id) do
  ...
  defp handle_mcp_message(%{"jsonrpc" => "2.0", "method" => "tools/list", "id" => id}, _org_id) do
  ...
  defp handle_mcp_message(%{"jsonrpc" => "2.0", "method" => "resources/list", "id" => id}, _org_id) do
    JsonRpc.success_response(id, %{"resources" => []})
  end

  defp handle_mcp_message(%{"jsonrpc" => "2.0", "method" => "ping", "id" => id}, _org_id) do
    JsonRpc.success_response(id, %{})
  end

  defp handle_mcp_message(%{"jsonrpc" => "2.0", "method" => "notifications/initialized"}, _org_id) do
    %{"jsonrpc" => "2.0"}
  end

  defp handle_mcp_message(%{"jsonrpc" => "2.0", "method" => "prompts/list", "id" => id}, _org_id) do
    JsonRpc.success_response(id, %{"prompts" => []})
  end

  defp handle_mcp_message(%{"jsonrpc" => "2.0", "method" => _method, "id" => id}, _org_id) do
    JsonRpc.error_response(id, -32_601, "Method not found")
  end

  defp handle_mcp_message(%{"jsonrpc" => "2.0"}, _org_id) do
    %{"jsonrpc" => "2.0"}
  end

  defp handle_mcp_message(_message, _org_id) do
