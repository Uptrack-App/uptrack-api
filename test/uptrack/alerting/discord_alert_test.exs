defmodule Uptrack.Alerting.DiscordAlertTest do
  use Uptrack.DataCase

  alias Uptrack.Alerting.DiscordAlert
  ...
  describe "send_incident_alert/3" do
    test "returns error when no webhook URL configured" do
      ...
      assert {:error, "No Discord webhook URL configured"} =
               DiscordAlert.send_incident_alert(channel, build_incident(), build_monitor())
    end
  ...
  describe "send_resolution_alert/3" do
    test "returns error when no webhook URL configured" do
      ...
      assert {:error, "No Discord webhook URL configured"} =
               DiscordAlert.send_resolution_alert(channel, incident, build_monitor())
    end
  end
  ...
  describe "send_test_alert/1" do
    test "returns error when no webhook URL configured" do
      channel = build_channel(%{})
      assert {:error, "No Discord webhook URL configured"} = DiscordAlert.send_test_alert(channel)
    end
  end
end
